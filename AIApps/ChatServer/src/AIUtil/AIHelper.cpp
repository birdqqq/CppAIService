...
AIHelper::AIHelper() {
    //默认使用阿里云大模型
    //1 对应阿里云大模型
    // StrategyFactory::instance() 获取单例工厂对象，create("1") 创建默认的AI策略对象
...
void AIHelper::addMessage(int userId,const std::string& userName, bool is_user,const std::string& userInput, std::string sessionId) {
    ...
    // 计算从1970年1月1日到现在的时间间隔 纳秒级别
    auto duration = now.time_since_epoch();
    // 将高精度的纳秒截取成毫秒， 使用count()将这个时间对象转变为long long 放进数据库
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
    // 将用户内容和时间戳放进AIHelper的内存容器中，使得AI有短期记忆
...
void AIHelper::restoreMessage(const std::string& userInput,long long ms) {
    // 将聊天消息从数据库加载到message中 比如重启服务器 内存清空，这个函数帮助AI恢复记忆
...
json AIHelper::executeCurl(const json& payload) {
    ...
    // HTTP对body的要求是纯文本  dump()是将JSON序列转化为字符串
    ...
    // res 放的是这次请求成功与否的状态码（是curl自己的状态码）
    CURLcode res = curl_easy_perform(curl);
    // 清理资源 非常重要！
    // 上面申请的header链表资源是手动分配的内存 必须使用curl_slist_free_all()释放 否则内存泄漏
    curl_slist_free_all(headers);
    //  curl对象同理 需要使用curl_easy_cleanup()释放
    ...
    try {
        // 返回服务器返回的内容（纯文本） 使用parse()将纯文本转换为json
...
size_t AIHelper::WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    // libcurl的接口是通用的c接口 只能接受void*,所以回调函数需要将其转换为真是类型才能使用
    std::string* buffer = static_cast<std::string*>(userp);
    // 将收到的数据追加到buffer中 ， 这里的buffer就是传进来的readBuffer
    buffer->append(static_cast<char*>(contents), totalSize);
    // 返回本次数据的大小，“告诉curl我处理了多少字节”  libcurl规定：回调函数必须返回你实际处理（消费）的字节数
    return totalSize;
}
