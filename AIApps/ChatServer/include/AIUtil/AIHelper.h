...
#include <vector>
#include <utility>
#include <curl/curl.h>   // 用于发送HTTP请求 例如：服务器 HTTP POST  请求 阿里云大模型API 通过curl库发送请求
#include <iostream>
#include <sstream>
...
#include"AIFactory.h"          // 使用工厂模式 用于创建不同的AI策略
#include"AIConfig.h"           // 用于加载配置文件 保存API_Key、URL、MODEL_NAME等信息
#include"AIToolRegistry.h"     // 支持工具调用的注册表，允许AI调用外部工具
...
private:
    std::string escapeString(const std::string& input); // 对字符串进行转义，防止SQL注入和语法错误
...
private:
    ...
    */
    ...
    //后者代表时间戳
    std::vector<std::pair<std::string, long long>> messages;  // 聊天记录，存放用户和AI的对话内容及时间戳
